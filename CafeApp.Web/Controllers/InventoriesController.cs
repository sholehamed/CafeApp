...
namespace CafeApp.Web.Controllers
{
    ...
    public class InventoriesController : ControllerBase
    {
        ...
        public InventoriesController(IApplicationUnit applicationUnit)
        {
            _service = applicationUnit.Inventories;
        }
        ...
        [HttpGet("{id}")]
        public async Task<ActionResult<InventoryDto>> Get(Guid id)
        {
            try
            {
                var res = _service.GetBy(new GetInventoryParameter(id).GetSpecifications());
        ...
        [HttpPost("writesync")]
        public async Task<IActionResult> WriteSync(InventoryEntity entity)
        {
            try
            {
